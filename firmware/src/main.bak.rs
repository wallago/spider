...
fn run() -> Result<(), Error> {
    ...
    let credit_spi = SpiDeviceDriver::new(&bus, Some(pins.gpio36), &spi_config)?;
    let status_spi = SpiDeviceDriver::new(&bus, Some(pins.gpio26), &spi_config)?;
    ...
    let mut displays = [
        ...
        .reset_pin(PinDriver::output(pins.gpio54)?)
        .init(&mut Ets)
        ...
        Builder::new(
            ...
            SpiInterface::new(
                status_spi,
                PinDriver::output(pins.gpio48)?,
                &mut status_buffer,
            ),
        )
        .invert_colors(ColorInversion::Inverted)
        .reset_pin(PinDriver::output(pins.gpio53)?)
        .init(&mut Ets)
